...
6: Plus1 Frame Stop Count Analysis"""
...
def run_optimization(clean_seq, protein_seq, choice): # Full version
    ...
    try:
        ...
        chart_image_to_embed_in_excel = None # Initialize chart_image_to_embed_in_excel for this 'else' block
        if choice == 6:
            ...
            valid_chart_sizes = [chart_sizes[i] for i in range(len(chart_sizes)) if chart_sizes[i] > 0]
        ...
        if combine_outputs_var is not None and combine_outputs_var.get():
            ...
            # If accumulating, and it's choice 6, we might still want to generate the chart for Excel if that's a desired feature.
            # However, the prompt focuses on "combine_outputs_var is false" for plt.show().
            # For now, let's assume accumulation mode does not involve plt.show() or saving individual charts for Excel in this step.
            # The main save_accumulated_results_action would handle any aggregate charting if needed.
        else:
        ...
        if choice == 6:
            if sum(valid_chart_sizes) > 0: 
                try:
                    fig, ax = plt.subplots()
                    wedges, _, _ = ax.pie(valid_chart_sizes, colors=valid_colors, startangle=90, textprops={'fontsize': 9})
                    ax.legend(wedges, title="+1 Stop Codons", loc="center left", bbox_to_anchor=(1, 0.5), fontsize=8, title_fontsize=9)
                    ax.axis('equal')
                    plt.tight_layout()
                    plt.show()
                    ...
                    # No chart_image_to_embed_in_excel is set here, so it remains None for save_output_to_excel
                except Exception as e_chart_gen:
                    logger.error(f"Chart display failed for choice 6: {e_chart_gen}")
            else:
                logger.info("No +1 stop codons to chart for choice 6 (when not accumulating)")
            ...
            output_filename = save_output_to_excel(data, filename_template_for_individual_save, chart_image_path=chart_image_to_embed_in_excel) # Pass None for choice 6
            
            if output_filename: messagebox.showinfo("Complete", f"Operation completed.\nSaved to: {output_filename}", parent=parent_window)
            
            if chart_image_to_embed_in_excel and os.path.exists(chart_image_to_embed_in_excel): # Cleanup if any chart was saved for excel
                try:
...
6: Plus1 Frame Stop Count Analysis"""
        ...
        if results:
            ...
            if choice == 6:
                ...
                preferred_order = [
                    'Plus1_TGA_Count', 'Plus1_TAA_Count', 'Plus1_TAG_Count', 
                    'Plus1_Total_Stops', 'Sequence_Name'
                ]
...
def create_main_menu(): # Full version
    ...
    root.title("DNA Codon Optimization Tool - v1"); root.geometry("600x550")
